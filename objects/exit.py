...
class Exit(Object, EvenniaExit):
    ...
    def at_desc_scent(self, looker):
        if not looker.location or (not looker.location == self.location and not looker.location == self):
            return()
        if looker.location:
            looker.location.msg_contents(self.objsub('&1N is sniffing in the direction of &0d.', looker), exclude=[self, looker])

    def at_desc_flavor(self, looker):
        if not looker.location or (not looker.location == self.location and not looker.location == self):
            return()
        if looker.location:
            looker.location.msg_contents(self.objsub('&1N is tasting things near &0d.', looker), exclude=[self, looker])

    def at_desc_texture(self, looker):
        if not looker.location or (not looker.location == self.location and not looker.location == self):
            return()
        if looker.location:
            looker.location.msg_contents(self.objsub('&1N is feeling things near &0d.', looker), exclude=[self, looker])
