"""
The command template for the default MUCK-style command set. There
is also an OOC version that makes sure caller is a Player object.

This is designed to be compatible with the MuxCommand template
"""
...
class MuckCommand(Command):
    """
    This sets up the basis for a MUX command. The idea
    is that most other Muck-related commands should just
    ...
    def has_perm(self, srcobj):
        ...
        """
        return super(MuckCommand, self).has_perm(srcobj)
    ...
    def parse(self):
        ...
        A MUCK command has the following possible syntax:
	...
	while args.startswith('#'):
	   splitargs = args[1:].split(None, 1)
	   switches.append(splitargs[0])
	   if len(splitargs) > 1:
	       args = splitargs[1]
	   else:
	       args = ""
    ...
    def func(self):
        ...
        string += "\n" + "-" * 50
        string +=  "\nVariables from MuckCommand baseclass\n"
        string += "-" * 50
